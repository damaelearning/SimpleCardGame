...
def threading_ismcts_action(state):
    class Node:
        ...
        def evaluate(self, state, executor):
            if state.is_done():
                self.n += 1
            ...
            if not self.child_nodes:
                ...
                if self.n == 10:
                    self.expand()
                future = executor.submit(playout, state)
                self.w += reward
                ...
                reward = next_child_node.evaluate(next_state, executor)
                reward = reward if action != PASS_NUM else -reward
    ...
    def develop_game_tree(root_node, state):
        root_node.evaruate(state)
    ...
    with concurrent.futures.ThreadPoolExecutor() as te:
        with concurrent.futures.ProcessPoolExecutor() as executor:
            for i in range(1000):
                future = te.submit(develop_game_tree, root_node, state, executor)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if state.turn_owner.is_first_player:
            time1 = time.time()
            state = state.next(threading_ismcts_action(state))
            time2 = time.time()
